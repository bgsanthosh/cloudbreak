...
public class StorageConsumptionJobService {
    ...
    private static final String JOB_NAME = "storage-consumption-job";
    ...
    private Date delayedFirstStart() {
        return Date.from(ZonedDateTime.now().toInstant().plus(Duration.ofMinutes(RANDOM.nextInt(storageConsumptionConfig.getIntervalInMinutes()))));
    }
}