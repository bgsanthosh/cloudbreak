...
class SdxDeleteServiceTest {
    ...
    void deleteSdxClustersForEnvironment() {
        ...
        when(crnAwareSdxConnector.getAttemptResultForDeletion(any(), any(), any())).thenReturn(AttemptResults.finishWith(null));
        underTest.deleteSdxClustersForEnvironment(pollingConfig, environment, true);
        verify(crnAwareSdxConnector).delete(eq(sdxCrn1), eq(true));
        verify(crnAwareSdxConnector).delete(eq(sdxCrn2), eq(true));
        verifyNoMoreInteractions(crnAwareSdxConnector);
    }
    ...
    void deleteSdxClustersForEnvironmentFail() {
        ...
        String sdxCrn2 = "crn:cdp:sdxsvc:us-west-1:tenant:instance:crn2";
        when(environmentResourceDeletionService.getAttachedSdxClusterCrns(environment)).thenReturn(Set.of(sdxCrn1, sdxCrn2));
        when(crnAwareSdxConnector.getAttemptResultForDeletion(any(), any(), any())).thenReturn(AttemptResults.breakFor(new IllegalStateException()));
